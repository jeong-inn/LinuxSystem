...
int make_tokens(char *str, char tokens[TOKEN_CNT][MINLEN])
{
	...
	while(1)
	{
		...
		rtrim(tokens[row]);
		{
			char *trimmed = ltrim(tokens[row]);
			if(trimmed != tokens[row])
				memmove(tokens[row], trimmed, strlen(trimmed) + 1);
		}
	...
	for(i = 0; i < (int)strlen(start); i++)   
	{
		...
		else
		{
			...
			if(start[i] == '.' && i < (int)strlen(start)){
				while(start[i + 1] == ' ' && i < (int)strlen(start))
					i++;
	...
	if(row > 0)
	{
		...
		if(strcmp(tokens[0], "#include") == 0 || strcmp(tokens[0], "include") == 0 || strcmp(tokens[0], "struct") == 0){ 
			char *normalized;
			clear_tokens(tokens); 
			normalized = remove_extraspace(str);
			if(normalized == NULL)
				return false;
			strncpy(tokens[0], normalized, MINLEN - 1);
			free(normalized);
		}
	}
		...
		else if(is_operator(tokens[*idx]))
		{
					...
					|| !strcmp(tokens[*idx], "+") || !strcmp(tokens[*idx], "*"))
			{
				if(is_operator(cur->name) == true && !strcmp(cur->name, tokens[*idx])) {
					/* cur already represents this associative operator. */
				}
...
char *rtrim(char *_str)
{
	...
	end = _str + strlen(_str);
	while(end > _str && isspace((unsigned char)end[-1]))
		--end;
...
{
	int i;
	char *str2 = (char*)calloc(BUFLEN, sizeof(char));
	char *start, *end;
	char temp[BUFLEN] = "";
	int position;
